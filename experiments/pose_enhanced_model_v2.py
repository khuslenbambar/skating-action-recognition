...
class PoseEnhancedModel(nn.Module):
    ...
    def forward(self, video_clip, pose_sequence):
        ...
        action_logits = self.action_head(fused)  # (batch, num_classes)

        # Quality assessment: sigmoid maps to [0,1], then scale to [1,10]
        quality_raw = self.quality_head(fused)  # (batch, 1)
...
class PoseEnhancedTrainer:
    ...
    def __init__(self, model, train_loader, val_loader, device, config):
        ...
        # Count samples per class to handle imbalance
        # (e.g., 4Flip has ~5 videos while 2Axel has hundreds)
        ...
        # Loss functions — weighted CrossEntropy for classification
        ...
        self.quality_weight = config.get("quality_loss_weight", 0.1)  # Was 0.5
