...
IMPROVED VERSION — Changes from original (all in PoseEnhancedTrainer):
    ...
    Model architecture, dataset class, and pose encoder are UNCHANGED.

Architecture:
    ┌─────────────────┐     ┌─────────────────┐
    │   Video Clip     │     │  Pose Sequence   │
    │  (3,16,112,112)  │     │   (16, 17, 3)    │
    └────────┬────────┘     └────────┬────────┘
             │                       │
             ▼                       ▼
    ┌─────────────────┐     ┌─────────────────┐
    ...
    └────────┬────────┘     └────────┬────────┘
             ...
             └───────┬───────────────┘
              ...
              ▼              ▼
    ┌──────────────┐  ┌──────────────┐
    │ Action Head   │  │ Quality Head  │
    │ (N classes)   │  │ (score 1-10)  │
    └──────────────┘  └──────────────┘
...
# ImageNet normalisation (same as baseline)
IMAGENET_MEAN = [0.43216, 0.394666, 0.37645]
IMAGENET_STD  = [0.22803, 0.22145, 0.216989]
NUM_KEYPOINTS = 33
KEYPOINT_DIM = 3

# Granularity mappings (mirrors skating_action_recognition.py)
GRANULARITY_COARSE = {
...
# =============================================================================
# Section 1: Pose Encoder Network (UNCHANGED)
# =============================================================================
...
class PoseEncoder(nn.Module):
    ...
    def __init__(self, num_keypoints=17, keypoint_dim=3, hidden_size=128,
        ...
        # Project raw keypoints to a higher dimension
...
# =============================================================================
# Section 2: Pose-Enhanced Model (UNCHANGED)
# =============================================================================
...
class PoseEnhancedModel(nn.Module):
    ...
    def __init__(self, num_classes, rgb_feature_dim=512, pose_feature_dim=128,
        ...
        # ---- Late-fusion heads (A5): two separate classifiers, logits averaged ----
    ...
    def forward(self, video_clip, pose_sequence):
        ...
        Parameters
        ----------
        video_clip : torch.Tensor — Shape (batch, 3, T, 112, 112)
        pose_sequence : torch.Tensor — Shape (batch, T, 17, 3)

        Returns
        -------
        action_logits : torch.Tensor — Shape (batch, num_classes)
        quality_score : torch.Tensor — Shape (batch, 1), range [1.0, 10.0]
        """
        # ---- A1: Pose-only mode — bypass R3D-18 entirely ----
        if self.pose_only:
            pose_features = self.pose_encoder(pose_sequence)  # (batch, 128)
            action_logits = self.pose_only_head(pose_features)
            # Dummy quality score (unused when --no_quality_head; benign otherwise)
        ...
        # RGB pathway
        rgb_features = self.rgb_backbone(video_clip)      # (batch, 512)
        # Pose pathway
        pose_features = self.pose_encoder(pose_sequence)  # (batch, 128)

        # ---- A5: Late fusion — average logits from two separate heads ----
        if self.fusion_mode == "late":
            ...
            # Quality head still uses concat fusion (it's a small head, harmless)
        ...
        # ---- Default: concat fusion (current v2 behavior) ----
        fused = torch.cat([rgb_features, pose_features], dim=1)  # (batch, 640)
...
# =============================================================================
# Section 3: Dataset (UNCHANGED)
# =============================================================================

class PoseEnhancedDataset(Dataset):
    """
    Dataset that loads video clips AND their corresponding pose sequences.
    """
    ...
    def __init__(self, video_root, pose_root, clip_length=16,
        ...
        # Discover element-level classes from folder structure
        ...
        # Build sample list with mapped target labels
        self.samples = []
...
# =============================================================================
# Section 4: Training Loop — IMPROVED VERSION
# =============================================================================

class PoseEnhancedTrainer:
    """
    Training loop for the dual-head pose-enhanced model.

    IMPROVEMENTS over original:
        1. AdamW optimizer (proper decoupled weight decay)
        2. Early stopping with configurable patience
        3. Gradient clipping to prevent exploding gradients
        4. Better logging (shows current LR, early stopping countdown)
    """
    ...
    def __init__(self, model, train_loader, val_loader, device, config):
        ...
        # --- CHANGE 5: Compute class weights from training data ---
        # (Toggleable via config['no_class_weights'] for ablation A2)
        ...
        # A3: --no_quality_head forces quality_weight=0 (skips quality contribution to loss)
        ...
        # --- CHANGE 1: AdamW instead of Adam ---
        # AdamW applies weight decay correctly (decoupled from gradient),
        # which improves regularisation compared to Adam + L2
        ...
        # Learning rate scheduler (unchanged — already ReduceLROnPlateau)
        ...
        # --- CHANGE 3: Gradient clipping threshold ---
    ...
    def _compute_quality_targets(self, action_logits, labels):
        """
        Generate pseudo quality scores using model confidence as proxy.
        Higher confidence in the correct class → higher quality score.

        Returns quality_targets in range [1.0, 10.0]
        """
        with torch.no_grad():
            probs = torch.softmax(action_logits.detach(), dim=1)
            correct_probs = probs.gather(1, labels.unsqueeze(1))  # (batch, 1)
    ...
    def train_one_epoch(self):
        ...
        for clips, poses, labels in self.train_loader:
            ...
            # Quality loss (using pseudo labels)
            ...
            # --- CHANGE 3: Gradient clipping before optimizer step ---
